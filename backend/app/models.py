...
class Cosmetic(Base):
 __tablename__='cosmetics'; id:Mapped[int]=mapped_column(primary_key=True); sku:Mapped[str]=mapped_column(String(100),unique=True); name:Mapped[str]=mapped_column(String(200)); price:Mapped[float]=mapped_column(Float); kind:Mapped[str]=mapped_column(String(32)); metadata_json:Mapped[dict]=mapped_column(JSON,default=dict)
class UserCosmetic(Base):
 __tablename__='user_cosmetics'; id:Mapped[int]=mapped_column(primary_key=True); user_id:Mapped[int]=mapped_column(ForeignKey('users.id'),index=True); cosmetic_id:Mapped[int]=mapped_column(ForeignKey('cosmetics.id'),index=True); equipped:Mapped[bool]=mapped_column(Boolean,default=False); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
class SocialPost(Base):
 __tablename__='social_posts'; id:Mapped[int]=mapped_column(primary_key=True); user_id:Mapped[int]=mapped_column(ForeignKey('users.id'),index=True); body:Mapped[str]=mapped_column(Text); boost_percent:Mapped[float]=mapped_column(Float,default=0); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
class SocialFollow(Base):
 __tablename__='social_follows'; id:Mapped[int]=mapped_column(primary_key=True); follower_id:Mapped[int]=mapped_column(ForeignKey('users.id'),index=True); followed_id:Mapped[int]=mapped_column(ForeignKey('users.id'),index=True); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
