...
def load_config(path: str | Path | None = None):
    ...
    if not p.exists():
        raise RuntimeError(f'Missing config file: {p}')
    ...
    c.target_min = float(data['difficulty']['target_min_accuracy']); c.target_max = float(data['difficulty']['target_max_accuracy'])
    c.decay_rate = float(data['decay']['rate_percent']); c.decay_frequency_hours = int(data['decay']['frequency_hours'])
    c.boss_time_per_turn = int(data['boss_battle']['default_time_per_turn_seconds'])
