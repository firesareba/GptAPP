...
class AttemptIn(BaseModel):
    user_id: str; problem_id: int; submitted_answer: str; elapsed_ms: int = Field(ge=0); raw_grading_response: dict
...
async def attempt(a: AttemptIn):
    async with Session() as db:
        ...
        g = a.raw_grading_response
        if g.get('problem_id') != p.id or g.get('submitted_answer') != a.submitted_answer or 'correct' not in g: raise HTTPException(422, 'Invalid grading evidence')
        correct = bool(g['correct']); db.add(Attempt(user_id=u.id, problem_id=p.id, submitted_answer=a.submitted_answer, correct=correct, grading_evidence=g, elapsed_ms=a.elapsed_ms))
